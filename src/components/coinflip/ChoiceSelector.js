...
const ChoiceSelector = memo(({ selectedSide, onSelectSide }) => {
  return (
    <div className='flex flex-row w-full h-auto gap-3 justify-between items-center'>
      <div
        className={`flex flex-col ${selectedSide === 'heads' ? 'bg-[#f2c521]' : 'bg-[#A2A2A296]'} rounded-[26px] py-3 px-5 items-center justify-center w-[45%]
        transition-all duration-300 ease-in-out hover:scale-105 hover:shadow-lg cursor-pointer
        group relative overflow-hidden animate-coin-pulse hover:bg-[#F2C521]`}
      ...
      >
        ...
        <span className="font-['Bowlby_One'] font-normal text-sm md:text-[20px] whitespace-nowrap leading-[100%] tracking-normal text-center align-middle
          ...
          HEAD ({selectedSide === 'heads' ? 'x2' : 'x0'})
        </span>
      </div>
      <div
        className={`flex flex-col ${selectedSide === 'tails' ? 'bg-[#f2c521]' : 'bg-[#A2A2A296]'} rounded-[26px] py-3 px-5 items-center justify-center w-[45%]
        transition-all duration-300 ease-in-out hover:scale-105 hover:shadow-lg cursor-pointer
        group relative overflow-hidden animate-coin-pulse hover:bg-[#F2C521]`}
      ...
      >
        ...
        <span className="font-['Bowlby_One'] font-normal text-sm md:text-[20px] whitespace-nowrap leading-[100%] tracking-normal text-center align-middle
          ...
          TAILS ({selectedSide === 'tails' ? 'x2' : 'x0'})
        </span>
      </div>
    </div>
  );
